...
let create_editor_with_new_mode () =
  ...
  let keywordsMapper value =
    if List.mem value keywords then Keyword
    else if List.mem value builtinConstants then Constant_language
    else if List.mem value builtinFunctions then Support_function
    else if value = "this" then Variable_language
    else Identifier
  in
  let keywords_regex = jsnew Js.regExp(Js.string "[a-zA-Z_$][a-zA-Z0-9_$]*") in
  let state_start = tokenizerState "start"
    [ Fun keywordsMapper, keywords_regex, None ]
  in
  let rules = tokenizerRules [ state_start ] in
